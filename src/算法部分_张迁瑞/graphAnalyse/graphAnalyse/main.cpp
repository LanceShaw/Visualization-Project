...
//连通分量,使用了广度优先搜索的方法
void ConnectedComponent(int n, const vector<vector<double>>& graph, double limit, vector<int>&start, vector<int>&end)
{
	int count = 0;
...
int main()
{
	...
	*/
	//求连通分量
	vector<int> start1(edgeNum);
	vector<int> end1(edgeNum);
	double limit;
	cout << "输入连通分量的边阈值";
	cin >> limit;
	ConnectedComponent(nodeNum, graph, limit,start1,end1);
	/*
